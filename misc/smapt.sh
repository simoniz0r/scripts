...
smapt-help () {
    ...
    echo "-Si  - apt install - install packages"
    echo "-Sui - apt update && apt install - update packages list and install packages"
    echo "-Sr  - apt remove - remove packages"
    echo "-Sa  - apt autoremove - Remove automatically all unused packages"
    echo "-Sud - apt update - update list of available packages"
    echo "-Sug - apt upgrade - upgrade the system by installing/upgrading packages"
    echo "-Suu - apt update && apt upgrade - run apt update and then apt upgrade"
    echo "-Sfu - apt full-upgrade - fully upgrade the system by removing/installing/upgrading packages"
    echo "-Ses - apt edit-sources - edit the source information file"
    echo 
